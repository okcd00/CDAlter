...
#
#   filename : http_file_server.py
#   author   : chendian / okcd00@qq.com
#   date     : 2020-07-05
#   desc     : server in http service via PY3
# ==========================================================================
...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        fd = self.send_head()
        if fd:
            self.copyfileobj(fd, self.wfile)
            fd.close()
    ...
    def list_directory(self, path):
        ...
        f = StringIO()
        display_path = html.escape(urllib.request.unquote(self.path))
        f.write('<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 3.2 Final//EN">')
        ...
        for name in list_dir:
            ...
            f.write('<li><a href="%s">%s</a>\n' % (ur.quote(linkname), html.escape(display_name)))
        f.write("</ul>\n<hr>\n</body>\n</html>\n")
        length = f.tell()
        f.seek(0)
        self.send_response(200)
...
def translate_path(path):
    ...
    path = path.split('?', 1)[0]
    path = path.split('#', 1)[0]
    path = posixpath.normpath(urllib.request.unquote(path))
    words = path.split('/')
